...
export default function NavButtons({type, projectId}: {type: string, projectId?: number}) {
    ...
    return (
        <S.LinkButtonContainer>
            ...
            {type === "releases" && (
                <S.ReleaseButton
                    onClick={() => {router.push(`/IssueBoard/${projectId}`)}}
                >
                    Issues
                </S.ReleaseButton>
            )}
            {type === "issues" && (
                <S.IssueButton>Releases</S.IssueButton>
            )}
