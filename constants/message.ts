export const SIGNUP_FORM_MESSAGE = {
  NAME_WARNING: "이름을 입력해주세요",
  EMAIL_WARNING: "이메일을 입력해주세요",
  CHECK_PASSWORD: "비밀번호를 확인해주세요",
  SIGNUP_SUCEESS: "회원가입 되었습니다.",
  SIGNUP_FAIL: "회원가입에 실패하였습니다.",
};

export const RELEASE_MESSAGE = {
  MEMBER_CANNOT_CREATE: "멤버는 릴리즈 노트를 생성할 수 없습니다.",
  LEAVE_RELEASE: "릴리즈 노트 창에서 나가시겠습니까?",
  CANCEL_RELEASE_CREATE: "정말로 릴리즈 노트 생성을 취소하시겠습니까?",
  CREATE_RELEASE_SUCCESS: "새로운 릴리즈 노트가 생성되었습니다.",
  LEAVE_RELEASE_EDIT: "릴리즈 수정창에서 나가시겠습니까?",
};

export const EMAIL_CONFIRM_FORM_MESSAGE = {
  EMAIL_CONFIRM_SUCCESS: "이메일 인증에 성공하였습니다.",
  WRONG_CONFIRM_NUMBER: "잘못된 인증번호 입니다.",
  EMAIL_SENT: "이메일이 전송되었습니다.",
  ENTER_EMAIL: "이메일을 입력해주세요",
};

export const FIND_PASSWORD_FORM_MESSAGE = {
  NAME_WARNING: "이름을 입력해주세요",
  EMAIL_WARNING: "이메일을 입력해주세요",
  NEW_PASSWORD_WARNING: "새로운 비밀번호를 입력해주세요",
  CHECK_NEW_PASSWORD: "비밀번호를 확인 해주세요",
  SET_NEW_PASSWORD_SUCCESS: "비밀번호가 재설정 되었습니다.",
};
